...
export class PhysicsCar {
  ...
  private applyControls(controls: CarAIOutput, deltaTime: number): void {
    ...
    if (controls.brake) {
      ...
      // If moving forward at moderate speed, apply braking
        ...
        const brakeFactor = Math.min(
          1,
          0.7 + (currentSpeed / this.maxSpeed) * 0.3,
        );
        ...
        const brakeRatio = Math.max(
          0,
          1 - brakeForceMagnitude * deltaTime * 10,
        );
  ...
  private updateSteering(controls: CarAIOutput, deltaTime: number): void {
    ...
    if (this.currentSteeringAngle < targetAngle) {
      this.currentSteeringAngle = Math.min(
        targetAngle,
        this.currentSteeringAngle + steeringDelta,
      );
  ...
  private updateWheels(): void {
    ...
    Matter.Body.setAngle(
      this.wheels.frontLeft,
      carAngle + this.currentSteeringAngle,
    );
    Matter.Body.setAngle(
      this.wheels.frontRight,
      carAngle + this.currentSteeringAngle,
    ...
    if (Math.abs(this.currentSteeringAngle) > 0.01) {
      ...
      if (speed > 0.1) {
        ...
        const forwardVelocity =
          carForwardX * velocity.x + carForwardY * velocity.y;
        ...
        const newVelocityX =
          velocity.x * (1 - wheelGrip) +
          carForwardX * forwardVelocity * wheelGrip;
        const newVelocityY =
          velocity.y * (1 - wheelGrip) +
          carForwardY * forwardVelocity * wheelGrip;
  ...
  isDrifting(): boolean {
    ...
    const dotProduct =
      forwardX * normalizedVelocityX + forwardY * normalizedVelocityY;
